...
standard `npx @jfrog/agent-guard` shape with the **reserved upstream
name** `jfrog-plugin-mcp` in `_JF_ARGS` (see `.mcp.json`).
...
fallback. The only supported removal is `/plugin uninstall jfrog`.
