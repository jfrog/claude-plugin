...
The script reads `paths` and optional `keep` from `sync-modules-vendor.json` (today: `paths: ["modules"]`) and replaces the whole `modules/` tree, then restores any `keep` files that existed before the sync.
