...
- **`/reload-plugins`** — reconnects MCP servers in the current session, no
  restart. Use this when every value reference the entry needs was already
  exported in the shell that launched Claude Code (so nothing new needs to be
  inherited into the process).
- **`/exit` and relaunch `claude`** in the same directory — required if a value
  reference was exported (or changed) in the shell AFTER Claude Code was
  launched; a running process never picks up its launching shell's later
  exports, so only a fresh launch inherits them. On first launch accept the
