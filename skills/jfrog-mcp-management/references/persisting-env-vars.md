...
- After exporting, the user must **relaunch the agent** so `${VAR}`
  references resolve — a running process never picks up a shell export made
  after it started, so `/reload-plugins` is NOT enough here even on Claude
  Code (see [harness-claude.md](harness-claude.md) Restart); only a full
  `/exit` + relaunch inherits the new value.
