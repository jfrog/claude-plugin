...
**Prerequisite — Step 0.** Before any download (`jf agent plugins install` /
`update`, or the Agent Guard versions lookup that resolves `--repo`), run the
Agent Guard activation check and interpret the exit code per
[agent-guard-activation.md](agent-guard-activation.md). Do not install or update
on a non-zero exit (except the listed exception).
